...
namespace ticl {
  ...
  using SimToRecoCollectionT = edm::AssociationMap<
	edm::OneToManyWithQualityGeneric<CaloParticleCollection, CLUSTER, std::pair<float, float>>>;
  template <typename CLUSTER>
  using RecoToSimCollectionT = edm::AssociationMap<
	edm::OneToManyWithQualityGeneric<CLUSTER, CaloParticleCollection, float>>;
